...
### AI Credit Assistant: confirmation before save

AI 信贷管家现在不仅能识别贷款 / 分期信息，还支持在保存到还款管理前进入 **保存前确认态**：
- 先集中核对产品名、当前应还、剩余待还、还款日、每期金额、APR / 年化等关键字段
- 对字段较完整的结果优先引导进入确认，而不是直接静默保存
- 当系统检测到可能对应已有负债时，会展示 **与已保存负债的差异**
- 用户可以选择：
  - 更新已有负债
  - 另存为新负债
  - 继续补充后再保存

这让 AI 信贷识别从“提取结果”进一步变成“可审阅、可分流、可落库”的流程，而不是一次性黑盒导入。
